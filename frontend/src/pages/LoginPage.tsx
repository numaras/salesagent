...
export default function LoginPage() {
  ...
  useEffect(() => {
    apiFetch<OidcStatus>("/oidc/config")
      .then((cfg) => {
        setOidcStatus(cfg);
        if (cfg.provider === "google" || cfg.oidc_enabled) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {googleConfigured && (
            <a
              href="/admin/api/auth/google"
              className="mb-3 flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50"
            >
          ...
          {oidcStatus?.oidc_enabled && (
            <button
              type="button"
              className="mb-3 w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50"
            >
              Sign in with SSO
            </button>
          )}

          {(googleConfigured || oidcStatus?.oidc_enabled) && (
